param(
    [string]$DataUrl = "https://huggingface.co/datasets/onejetpilot/autos/resolve/main/autos.csv"
)

# Останавливаем пайплайн при первой ошибке.
$ErrorActionPreference = "Stop"

# Шаг 1: ingest (скачивание autos.csv + manifest).
Write-Host "[pipeline] Step 1/4 ingest"
python .\src\ingest.py --data-url $DataUrl --output-dir data/raw
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

# Шаг 2: validate (проверка схемы и базовой целостности).
Write-Host "[pipeline] Step 2/4 validate"
python .\src\validate.py --raw-csv data/raw/autos.csv --output-dir data/validated
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

# Шаг 3: build_dataset (очистка, признаки, train/val).
Write-Host "[pipeline] Step 3/4 build_dataset"
python .\src\build_dataset.py --validated-csv data/validated/autos_validated.csv --output-dir data/processed
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

# Шаг 4: train (CatBoost + RMSE/MAE).
